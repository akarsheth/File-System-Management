...
class FileSystem
{
	...
	public static void main(String[] args) throws Exception
    {
    	...
    	file = new RandomAccessFile("C:\\Users\\Akar\\Desktop\\os.txt","rw");
    	for( int i=0 ; i<totalFileEntry ; i++ ) // for retreiving previous stored data when the program runs again
    	{
    	...
    	for( int i=0 ; i<totalfat ; i++ )   //check offfat is the offfset for going to FAT
    	{
    	...
    	while( true )
    	{
    		...
    		else if( s[0].equals("readf") )
    		{
    			if( index.containsKey(s[1]) )
    			{
    				char pp = getDataFromFileEntry(index.get(s[1]), offpermi, 1).charAt(0);  // return the string,charAt(0)
					int[] per = getPer(pp);
					if( per[0]==1 || per[1]==1 ) // Check for read permissions
					{
    		...
    		else if( s[0].equals("writef") )
    		{
    			if( s[1].equals("append") )
    			{
    				if( index.containsKey(s[2]) )
    				{
    					...
    					int[] per = getPer(pp);  //gives us the permissions for the file
    					if( per[1]==1 )
    					{
	...
	static void rename( String oldName, String newName ) throws IOException
	{
		...
		StringBuilder sb = new StringBuilder();
		for( int i=0 ; i<offcreated ; i++ )   //first clearing the 10 bytes of name with 0
			sb.append((char)0);
	...
	static void readFile( String name, StringBuilder con ) throws Exception   //Details obtained from file detail table
	{
		int ind = index.get(name);
		int firstCluster = basetodec(getDataFromFileEntry(ind,offfirstcluster,3));
		int endOfFile = basetodec(getDataFromFileEntry(ind, offEnd, 1));
    ...
    static int createFile( String name, char permissions ) throws Exception
    {
    	...
    	file.seek(address+offfirstcluster);   // check
